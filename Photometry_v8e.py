...
def main(date, objid, predmag, magrange, fwhmmultiplier, snr, apradius, anninner, annouter):
    ...
    # November 25, 2020 - All Median unless noted
    #targetsub = 'J005705.02+120635.5'
    
    TargetObject = objid+"/"
    date = date
    # TARGET_RAN ='93.324568228571' #AT2020znl
    # TARGET_DECN = '-15.892058514286'
    ...
    togglestacktoggle = 'Mean'
    displayoptions = 'All'
    #displayoptions = 'No'
    
    if(trigger=='off'):
    #   TOGGLESTACK = togglestacktoggle
        TOGGLESTACK = 'Median'
    ...
    targetpath= processedpath + '/' + TargetObject + date
    ...
    targetlistA=targetlistAblue+targetlistAred+targetlistAgreen+targetlistAg+targetlistAr \
    +targetlistAi+targetlistAu+targetlistAz+targetlistAlum+targetlistAHa
    print(targetlistA)
    
    targetlistB=targetlistAblue+targetlistAred+targetlistAgreen+targetlistAg+targetlistAr \
    +targetlistAi+targetlistAu+targetlistAz+targetlistAlum+targetlistAHa
    ...
    #            time.sleep(3)
             ...
             # Toggle='Yes'
            print('three') 
            print(target, " is an SDSS12 Catalog object")
            field_of_view =0.001
            Vizier.ROW_LIMIT = -1
            ToggleVal = Vizier.query_region(STAR_NAME, radius=field_of_view * u.deg, catalog = 'SDSS12') 
            if(ToggleVal is not None):
                targetobjresults=[]
                targetobj={}
                targetobj = Vizier.query_region(STAR_NAME, radius=field_of_view * u.deg, catalog = 'SDSS12') 
    #            print ('SDSS12', targetobj[0]['SDSS12'][0], '; RA = ', targetobj[0]['RA_ICRS'][0],'; DEC = ', targetobj[0]['DE_ICRS'][0],'; rmag = ', targetobj[0]['rmag'][0],'; umag = ', targetobj[0]['umag'][0],'; gmag = ', targetobj[0]['gmag'][0],\
    #                               ';imag = ', targetobj[0]['imag'][0],'; zmag = ', targetobj[0]['zmag'][0])
                targetobjresults.append({'auid':'SDSS12'+str(targetobj[0]['SDSS12'][0]),'ra': targetobj[0]['RA_ICRS'][0],'dec':targetobj[0]['DE_ICRS'][0], 'rmag': targetobj[0]['rmag'][0], 'umag': targetobj[0]['umag'][0], 'gmag': targetobj[0]['gmag'][0],\
                                                'imag': targetobj[0]['imag'][0], 'zmag': targetobj[0]['zmag'][0]})
    #
    ...
    for i in range(numtargetlist1):
        ...
        #
            ...
            # 
        #
            if(TARGET_FLUX_R>13 or filter2 == 'u'):   
                #
            ...
            #
                menu2 = menu1 
            #    
            # Second Filter of Results:  Section Filters for Stars, q-mode Quality Flag and Drops Duplicates if these Remain
            #
                resultx = []
                resultx = results.query('q_mode == "+"') 
                resultx = resultx.query('objecttype == 6') 
            #   
            #    resultx.drop_duplicates(subset='peak', keep='first', inplace=True)
                resultx = resultx.reset_index(drop=True)
            #      
                try:
                    target_magnitude1, residuals1, slope1, yintercept1 = get_regression(resultx,filter2)
            #
                    menu2[i]['Num Stars_QMode'] = len(resultx)
                    menu2[i]['Mag_Estimate_QMode'] = target_magnitude1
                    residuals1 = residuals1[0].item(0)
                    menu2[i]['Residuals_QMode'] = residuals1
                    stddev = math.sqrt(residuals1/(len(resultx)-1))
                    menu2[i]['QMode_Std_Dev'] = stddev
                except:
            ...
            #
                if(ResultsPrint == 'Yes'):
                    resultz2 = pd.DataFrame(resultx)
                                #
                    clock = datetime.today()
                    ID2 = str(clock.year) + '-' + str(clock.month) + '-' + str(clock.day) + '-' + 'h' + str(clock.hour) + 'm' + str(clock.minute) + 's' + str(clock.second)
                    name2 = 'Results2_' + STAR_NAME + '_' + ID2 + '-' + filter2 + '.xlsx'
                    resultz2 = resultz2[['auid','auidshort','rmag','gmag','imag','zmag','umag','quality','q_mode','objecttype','flags','flagint','Clean','instrumental_mag','residual_aperture_sum','peak','ra','dec','x','y']]
                    print(resultz2)
                    os.chdir(processedpath)
                    name2=name2.replace(" ","_")
                    name2=name2.replace("/","-")
                    name2=name2.replace("*","star")
                    resultz2.to_excel(name2)
            #    
            # Third Filter of Results:  Filter for Clean Photometry Stars
                menu3 = menu2 
                resulty = []
                resulty = results.query('Clean == "True"') 
                resulty = resulty.query('objecttype == 6')     
                resulty = resulty.reset_index(drop=True)
                #
            #    resulty.drop_duplicates(subset='peak', keep='first', inplace=True)
                try:
                    target_magnitude2, residuals2, slope2, yintercept2 = get_regression(resulty,filter2)
                    print("Got Back")
                #
                    menu3[i]['Num Stars_Clean'] = len(resulty)
                    menu3[i]['Mag_Estimate_Clean'] = target_magnitude2
                    residuals2 = residuals2[0].item(0)
                    menu3[i]['Residuals_Clean'] = residuals2
                    stddev = math.sqrt(residuals2/(len(resulty)-1))
                    menu3[i]['Clean_Std_Dev'] = stddev
                except:
                    ...
                    residuals2 = 'nan'
                    menu3[i]['Residuals_Clean'] = residuals2     
                    menu3[i]['Clean_Std_Dev'] = 'nan'
                #    
                if(ResultsPrint == 'Yes'):
                    resultz3 = pd.DataFrame(resulty)
                #
                    clock = datetime.today()
                    ID3 = str(clock.year) + '-' + str(clock.month) + '-' + str(clock.day) + '-' + 'h' + str(clock.hour) + 'm' + str(clock.minute) + 's' + str(clock.second)
                    name3 = 'Results3_' + STAR_NAME + '_' + ID3 + '-' + filter2 + '.xlsx'
                    resultz3 = resultz3[['auid','auidshort','rmag','gmag','imag','zmag','umag','quality','q_mode','objecttype','flags','flagint','Clean','instrumental_mag','residual_aperture_sum','peak','ra','dec','x','y']]
                    print(resultz3)
                    os.chdir(processedpath)
                    name3=name3.replace(" ","_")
                    name3=name3.replace("/","-")
                    name3=name3.replace("*","star")
                    resultz3.to_excel(name3)
                menu4 = menu3
            # Fourth Filter of Results:  Filter for both Clean and Q-Mode Photometry Stars
                resultq = []
                resultq = results.query('Clean == "True"') 
                resultq = resultq.query('q_mode == "+"') 
                resultq = resultq.query('objecttype == 6')     
                resultq = resultq.reset_index(drop=True)
                #
            #    resulty.drop_duplicates(subset='peak', keep='first', inplace=True)
                try:
                    target_magnitude3, residuals3, slope3, yintercept3 = get_regression(resultq,filter2)
                    print("Got Back Again")
                #
                    menu4[i]['Num Stars_Q & Clean'] = len(resultq)
                    menu4[i]['Mag_Estimate_Q & Clean'] = target_magnitude3
                    residuals3 = residuals3[0].item(0)
                    menu4[i]['Residuals_Q & Clean'] = residuals3
                    stddev = math.sqrt(residuals3/(len(resultq)-1))
                    menu4[i]['Q & Clean_Std_Dev'] = stddev
                except:
                    ...
                    residuals3 = 'nan'
                    menu4[i]['Residuals_Q & Clean'] = residuals3     
                    menu4[i]['Q & Clean_Std_Dev'] = 'nan'
                #    
                if(ResultsPrint == 'Yes'):
                    resultz4 = pd.DataFrame(resultq)
                #
                    clock = datetime.today()
                    ID4 = str(clock.year) + '-' + str(clock.month) + '-' + str(clock.day) + '-' + 'h' + str(clock.hour) + 'm' + str(clock.minute) + 's' + str(clock.second)
                    name4 = 'Results4_' + STAR_NAME + '_' + ID4 + '-' + filter2 + '.xlsx'
                    resultz4 = resultz4[['auid','auidshort','rmag','gmag','imag','zmag','umag','quality','q_mode','objecttype','flags','flagint','Clean','instrumental_mag','residual_aperture_sum','peak','ra','dec','x','y']]
                    print(resultz4)
                    os.chdir(processedpath)
                    name4=name4.replace(" ","_")
                    name4=name4.replace("/","-")
                    name4=name4.replace("*","star")
                    resultz4.to_excel(name4)
                menu5 = menu4
                resultfinal = []
                resultfinal = resultq
                ...
                try:
                    if(filter2 == 'r'): 
                        rmagstdev = math.sqrt(residuals3/(len(resultfinal)-1))
                        resultfinal['dev_from_regr'] = abs((resultfinal['rmag'] - (resultfinal['instrumental_mag']*slope3 + yintercept3))/rmagstdev)
                        standard = rmagstdev
                    elif(filter2 == 'g'):
                        gmagstdev = math.sqrt(residuals3/(len(resultfinal)-1))
                        resultfinal['dev_from_regr'] = abs((resultfinal['gmag'] - (resultfinal['instrumental_mag']*slope3 + yintercept3))/gmagstdev)
                        standard = gmagstdev
                    elif(filter2 == 'u'):
                        umagstdev = math.sqrt(residuals3/(len(resultfinal)-1))
                        resultfinal['dev_from_regr'] = abs((resultfinal['umag'] - (resultfinal['instrumental_mag']*slope3 + yintercept3))/umagstdev)
                        standard = umagstdev
                    elif(filter2 == 'i'):
                        imagstdev = math.sqrt(residuals3/(len(resultfinal)-1))
                        resultfinal['dev_from_regr'] = abs((resultfinal['imag'] - (resultfinal['instrumental_mag']*slope3 + yintercept3))/imagstdev)
                        standard = imagstdev
                    elif(filter2=='z'):
                        zmagstdev = math.sqrt(residuals3/(len(resultfinal)-1))
                        resultfinal['dev_from_regr'] = abs((resultfinal['zmag'] - (resultfinal['instrumental_mag']*slope3 + yintercept3))/zmagstdev)
                        standard = zmagstdev
                    #
                    ...
                    menu5[i]['Num Stars_Final'] = len(resultfinal)
                    menu5[i]['Mag_Estimate_Final'] = target_magnitude4
                    residuals4 = residuals4[0].item(0) # Had read residuals3 = fix needs to be proved
                    menu5[i]['Residuals_Final'] = residuals4
                    stddev = math.sqrt(residuals4/(len(resultfinal)-1))
                    menu5[i]['Final_Std_Dev'] = stddev
            ##
                except:
                    target_magnitude4 = 'nan'
                    menu5[i]['Num Stars_Final'] = len(resultfinal)
                    menu5[i]['Mag_Estimate_Final'] = target_magnitude4
                    residuals4 = 'nan'
                    menu5[i]['Residuals_Final'] = residuals4     
                    menu5[i]['Final_Std_Dev'] = 'nan'      
            # 
                if(ResultsPrint == 'Yes'):
        ...
        #        results = results.query('objecttype == 6')
                try:
                    target_magnitude, residuals, slope, yintercept = get_regression(results,filter2)
                #
                    menu1[i]['Observation Date'] = ObsDate
            ...
            print('Target Could Not Be Located for this Observation')
            target_magnitude = 'nan'
            filler = 'nan'
            menu1[i]['ap_radius'] = filler
            menu1[i]['ann_radius_inner'] = filler
            menu1[i]['ann_radius_outer'] = filler
            menu1[i]['Num Stars_QMode'] = filler
            menu1[i]['Num Stars NoFltr'] = filler
            menu1[i]['Num Stars_Final'] = filler
            menu1[i]['Num Stars_Q & Clean'] = filler
            menu1[i]['Num Stars_Clean'] = filler
            menu1[i]['Num Stars_QMode'] = filler
            menu1[i]['Mag_Estimate_NoFltr'] = filler
            menu1[i]['Mag_Estimate_Final'] = filler
            menu1[i]['Mag_Estimate_Q & Clean'] = filler
            menu1[i]['Mag_Estimate_Clean'] = filler
            menu1[i]['Mag_Estimate_QMode'] = filler
            menu1[i]['Residuals NoFltr'] = filler
            menu1[i]['Residuals_Final'] = filler
            menu1[i]['Residuals_Q & Clean'] = filler
            menu1[i]['Residuals_Clean'] = filler
            menu1[i]['Residuals_QMode'] = filler
            menu1[i]['NoFltr_Std_Dev'] = filler
            menu1[i]['Final_Std_Dev'] = filler
            menu1[i]['Q & Clean_Std_Dev'] = filler
            menu1[i]['QMode_Std_Dev'] = filler
            menu1[i]['Clean_Std_Dev'] = filler
            menu1[i]['Observation Date'] = ObsDate
            menu1[i]['Num Stars NoFltr'] = len(results)
            menu1[i]['Mag_Estimate_NoFltr'] = target_magnitude
            residuals = 'nan'
    ...
    print(menu7)
    try:
        os.mkdir('/home/david/ZowadaTransients/Photometry/'+TargetObject)
        print("Directory made")
    except FileExistsError:
        print("Directory already exists")
    os.chdir('/home/david/ZowadaTransients/Photometry/'+TargetObject)
    name=name.replace(" ","_")
